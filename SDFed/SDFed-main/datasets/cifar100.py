...
class Cifar100():
    ...
    def __init__(self, cfg):
        ...
        print("Train data class counts per client:")
        for client_id, class_count in traindata_cls_counts.items():
            print(f"Client {client_id}: {class_count}")

        print("Test data class counts per client:")
        for client_id, class_count in testdata_cls_counts.items():
            print(f"Client {client_id}: {class_count}")
