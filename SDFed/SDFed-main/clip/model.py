...
class ModifiedResNet_GLP_OT(nn.Module):
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        super().__init__()
        self.output_dim = output_dim
        self.input_resolution = input_resolution
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            assert current_trainer in ['CLIP','PROMPTFL','GL_SVDMSE','GL_SVDMSE_HE','GLP_OT','FEDPGP']
...
class VisionTransformer_GLP_OT(nn.Module):
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "GLP_OT":
                self.visual = ModifiedResNet_GLP_OT(
            ...
            if trainer == "GLP_OT":
                self.visual = VisionTransformer_GLP_OT(
                    input_resolution=image_resolution,
